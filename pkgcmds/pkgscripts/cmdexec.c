...
/*	from OpenSolaris "cmdexec.c	1.15	06/02/27 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)cmdexec.c	1.4 (gritter) 2/26/07
 */
...
{
	char tempout[] = "/var/tmp/temp1XXXXXX";
	char command[256];

	if (close(mkstemp(tempout)) < 0)
		return (-1);

	(void) snprintf(command, sizeof command,
			"%s -f %s <%s >%s", cmd, input, file, tempout);
	if (system(command))
		return (-1);

	(void) snprintf(command, sizeof command, "cp %s %s", tempout, file);
	if (system(command))
		return (-1);
